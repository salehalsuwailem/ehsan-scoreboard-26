...
**دفعٌ موثوق (لا خطأ "fetch first"):** قبل كل دفع يُعيد السير الآلي الضبط على أحدث حالة للفرع (`git reset --hard FETCH_HEAD`) ثم يُعيد توليد البيانات فوقها، فيكون الدفع دائماً سريع‑التقديم دون فقدان أي لقطة. وإن تقدّم الفرع البعيد بيننا لأي سبب، تُعاد المحاولة كاملةً حتى النجاح. كما يمنع `concurrency` عمل تشغيلين معاً. **تنبيه:** يجب أن يكون هذا هو الـ workflow الوحيد الذي يدفع إلى `main`؛ احذف أي workflow قديم يفعل ذلك لتفادي التعارض.
...
3. لا حاجة لأي إعداد — Vercel يكتشف Vite ويشغّل `npm run build` (= `vite build` فقط).
...
**فصل المسؤوليات:** المزامنة تحدث في مكان واحد فقط — GitHub Actions — الذي يلتزم `data.json` المحدّث كل 15 دقيقة. أما Vercel فيبني الملف الملتزَم فقط، لذا **لا يحتاج إلى السرّ `SHEETS_API_URL`**، ولا يفشل بناؤه أبداً بسبب الـ API. (لبناء محلي بأحدث بيانات: `npm run sync` ثم `npm run build`، أو `npm run sync:build`.)
...
**غيّرت خلية في الشيت ولم يتغيّر الموقع؟** انتظر دورة المزامنة التلقائية (حتى 15 دقيقة) أو شغّل **Run workflow**؛ ومحلياً `npm run sync`. تأكّد أن `SHEETS_API_URL` مضبوط في أسرار GitHub (لا يلزم على Vercel).
